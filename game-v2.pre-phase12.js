...
const state = {
  ...
  // Quick-Chat Presets (Phase 5 Task 9)
  quickChat: [
...
function setScreen(name) {
  ...
  const flow = ['lobby', 'setup', 'roles', 'loadout', 'briefing', 'mission', 'results', 'replay', 'spectator', 'tournament', 'mapEditor'];
...
const ChatSystem = {
  ...
  render() {
    ...
    el.innerHTML = filtered.map(([name, text, time, team]) => {
      ...
      // Team badge for team-only messages
      let teamBadge = '';
      if (team && !isSystem) {
        const teamClass = team === 'North' ? 'team-north' : 'team-south';
        teamBadge = `<span class="chat-team-badge ${teamClass}">${escapeHtml(team)}</span>`;
      }
      return `<p data-sender="${safeName}">${timeStr}${teamBadge}<span class="chat-name">${safeName}</span>${safeText}${muteBtn}</p>`;
    }).join('');
  ...
  renderTabs() {
    const allTab = $('#chatTabAll');
    const teamTab = $('#chatTabTeam');
    if (!allTab || !teamTab) return;
    allTab.classList.toggle('active', state.chatTab === 'all');
    teamTab.classList.toggle('active', state.chatTab === 'team');
  },

  setTab(tab) {
    state.chatTab = tab === 'team' ? 'team' : 'all';
    saveState();
    this.render();
  },
